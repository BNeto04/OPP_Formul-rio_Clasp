...
## Dependencia C01 -> C03/ARCA (card #137)
Foi definida a **porta canonica** para a condicao de dominio do titulo PIP de veiculo: `AdaptadorConsultaArca.consultarPorRuleId('ARCA-VEICULO-001')`, com fail-soft. Estado: **REAL desde o #138** — `Entrada/EntradaManual.js` expoe `obterMetadadosArcaVeiculo()` (porta `consultarPorRuleId`, fail-soft) e `Entrada/Formulario.html` consome o rotulo canonico (`tituloCanonicoVeiculoArca_`), com fallback local. Fronteira: a ARCA rege a CONDICAO de dominio; o parser segue responsavel por ler o BO e pela realizacao lexical (heuristica OCR). Planta Mestra atualizada no #138 com a aresta `C01 -> C03/ARCA` (consulta canonica do titulo de veiculo).
