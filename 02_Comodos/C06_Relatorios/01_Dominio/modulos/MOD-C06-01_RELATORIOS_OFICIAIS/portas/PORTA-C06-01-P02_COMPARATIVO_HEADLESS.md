...
| race_condition | pendente | **Justificativa (herdada e agravada):** e o segundo chamador da mesma escrita de nome fixo `COMPARATIVO_2026` — a prova headless pode rodar enquanto o operador gera pelo menu, sem `LockService` em nenhum arquivo de produto (unica mencao do repositorio: stub de sandbox em `Testes/TestMenuP3.js:80`). **Decisao exigida:** a mesma da P01. **Ate a decisao, nao executar a geracao headless em paralelo com a geracao pelo menu.** |
