...
|---|---|---:|
| Total de regras (`regras`) | registros da lista | **49** |
| `rule_id` únicos | valores distintos | **49** |
| Auditadas pelo Guardião (`MAPEADO`) | `auditabilidade_guardiao.status` | **33** |
| Aplicadas por outros componentes (`INTEGRADO`) | `auditabilidade_guardiao.status` | **8** |
| Estruturais (`NAO_APLICAVEL`) | `auditabilidade_guardiao.status` | **8** |
| Cegas (`NAO_AUDITAVEL`) | `auditabilidade_guardiao.status` | **0** |
| Fora da auditoria do Guardião | `regras_total − regras_mapeadas` | **16** |
| Códigos de diagnóstico | soma de `auditabilidade_guardiao.codigos` | **38** (em 30 regras) |
| Porta de consulta | códigos → `rule_id` distintos | **38 → 30** |

**Leitura:** não existem regras "cegas" (`NAO_AUDITAVEL` = 0). As **49** regras têm destino:
**33** o Guardião audita, **8** são aplicadas pelo NormalizadorEfetivo/plugins via porta ARCA (#127),
e **8** são estruturais (agregação, layout, entrada de dados) que por definição não geram diagnóstico de planilha.

## Fluxo (fluid flow)

```
  ENTRADA            NORMALIZACAO           PLANILHA            GUARDIAO                METRICAS
  Formulario   ->    NormalizadorEfetivo -> abas mensais  ->    33 diagnosticos ->   plugins
  EntradaManual      (porta ARCA #127)     (tuneis)           (auditoria)             (PIP/drogas/armas)
```

## MAPEADO — auditadas pelo Guardião (33)
...
- **Condição:** Tunel operacional com dois ou mais policiais vinculados.
- **Esperado:** Equipe ordenada por posto/graduacao (mais antigo primeiro) e, no empate, pela matricula mais antiga (menor numero).
- **Diagnóstico(s):** `ORDEM_ANTIGUIDADE_EQUIPE`
- **Consumidor(es) real(is):** `Core/Policiais.js`, `Motor/PoliticaMeritoArmas.js`, `Features/NormalizadorEfetivo.js`
- **Motivo (auditabilidade_guardiao):** Codigo de diagnostico emitido pelo Guardiao (invariante do tunel) com metadados ARCA via porta - cards #146/#147.
...
- **Esperado:** Total de armas = soma da coluna ARMA (col 12); QDT ARMAS ignorada em somas; QDT ARMAS deve ser idêntico para todos os participantes do túnel e igual à soma de ARMA do túnel (invariante de participação
...
- **Motivo (auditabilidade_guardiao):** Codigo de diagnostico emitido pelo Guardiao (QDT_ARMAS_DIVERGENTE_NO_TUNEL) com metadados ARCA via porta. Invariante definida no card #141: QDT ARMAS identico entre participantes e igual a soma de ARMA do tunel.
...
- **Consumidor(es) real(is):** `Features/CompiladorGxt.js`, `Motor/PoliticaMeritoArmas.js`, `Features/GuardiaoQualidade.js`
- **Motivo (auditabilidade_guardiao):** Codigo de diagnostico emitido pelo Guardiao com metadados ARCA via porta (regra do proprietario 12/09: artesanal nao entra na quantidade fisica ARMA).
...
- **Motivo (auditabilidade_guardiao):** Codigo de diagnostico emitido pelo Guardiao com metadados ARCA via porta.
...
- **Motivo (auditabilidade_guardiao):** Regra tecnica do proprio auditor: nao gera diagnostico; descreve comportamento da varredura (classificacao de saude / cobertura) e por isso nao tem codigo associado.
...
- **Categoria:** IDENTIDADE_OCORRENCIA · **Subdomínio:** ocorrencia · **Cobertura:** COBERTO · **Confiança:** ALTA
- **Condição:** A mesma identidade de ocorrência (MIKE canônico = apenas dígitos + BOE) aparece em mais de uma DATA, ou o mesmo BOE aparece com o MIKE em mais de uma grafia.
- **Esperado:** Diagnóstico ALERTA OCORRENCIA_FRAGMENTADA_POR_DATA em cada linha da mesma identidade com datas distintas; e ALERTA MIKE_FORMATO_NAO_CANONICO_OU_DUPLICADO em cada linha do mesmo BOE com grafias diverge
- **Diagnóstico(s):** `OCORRENCIA_FRAGMENTADA_POR_DATA`, `MIKE_FORMATO_NAO_CANONICO_OU_DUPLICADO`
- **Consumidor(es) real(is):** `Core/RegrasQualidade.js`, `Features/GuardiaoQualidade.js`
- **Motivo (auditabilidade_guardiao):** Códigos de diagnóstico emitidos pelo Guardião (invariante de identidade unitária da ocorrência) com metadados ARCA via porta - card #160.
...
- **Categoria:** OBRIGATORIEDADE · **Subdomínio:** ocorrencia · **Cobertura:** COBERTO · **Confiança:** ALTA
- **Condição:** Gravação de ocorrência: a coluna QTD O (D) deve valer 01 na primeira linha do túnel.
- **Esperado:** QTD O = '01' na primeira linha; vazio nas linhas filhas; vazio/0/1 normalizados para 01.
- **Diagnóstico(s):** `QTD_O_DIVERGENTE`
...
- **Motivo (auditabilidade_guardiao):** Regra tecnica da porta de consulta do proprio catalogo (nao gera diagnostico).

### `ARCA-TERRITORIO-001` — Determinação Territorial Canônica de AIS por Município e Bairro
- **Categoria:** TERRITORIALIDADE · **Subdomínio:** territorio · **Cobertura:** COBERTO · **Confiança:** ALTA
...
- **Diagnóstico(s):** `AIS_AUSENTE`, `AIS_DIVERGENTE`
...
- **Motivo (auditabilidade_guardiao):** Codigo de diagnostico emitido pelo Guardiao (invariante AIS gravado x base territorial canonica cidade+bairro) com metadados ARCA via porta - card #149.

## INTEGRADO — aplicadas por outros componentes (8)
...
- **Motivo (auditabilidade_guardiao):** Precedencia por menor N aplicada em Motor/PoliticaMeritoArmas.js e NormalizadorEfetivo (#127); no Guardiao aparece via merito de armas.

### `ARCA-DETIDOS-001` — DETIDOS: Conjunto Canônico e Gravação no Fato (primeira linha)
- **Categoria:** CONSISTENCIA_CAMPOS · **Subdomínio:** ocorrencia · **Cobertura:** COBERTO · **Confiança:** ALTA
- **Condição:** Túnel operacional com detidos/conduzidos informados no fato.
- **Esperado:** DETIDOS gravado na primeira linha (fato) do túnel; linhas filhas vazias; cabeçalho aceito por alias canônico.
- **Diagnóstico(s):** —
- **Consumidor(es) real(is):** `Core/Constantes.js`, `Entrada/EntradaManual.js`, `Core/LeitorPlanilhas.js`, `Leitura/Adaptador2026.js`
- **Motivo (auditabilidade_guardiao):** Conjunto canonico e gravacao no fato aplicados na entrada (Constantes + EntradaManual); nao e regra de auditoria automatica do Guardiao.
...
- **Motivo (auditabilidade_guardiao):** Aplicada em Plugins/Metricas/PluginEntorpecentes.js; consolidacao cumulativa de drogas fora do caminho de auditoria do Guardiao.
...
- **Motivo (auditabilidade_guardiao):** Aplicada pelo NormalizadorEfetivo (Features/NormalizadorEfetivo.js) via porta ARCA (#127); transformacao de dados, nao auditoria de planilha.
...
- **Motivo (auditabilidade_guardiao):** Aplicada pelo NormalizadorEfetivo (desambiguacao de nomes de guerra por menor N) via porta ARCA (#127).
...
- **Motivo (auditabilidade_guardiao):** Aplicada em Plugins/Metricas/PluginPrisoes.js; contabilizacao de procedimentos legais fora do caminho de auditoria do Guardiao.
...
- **Motivo (auditabilidade_guardiao):** Higienizacao de formato aplicada pelo NormalizadorEfetivo; o Guardiao audita apenas a ausencia (ARCA-MATRICULA-002).
...
- **Motivo (auditabilidade_guardiao):** Aplicada em Plugins/Metricas/PluginPontuacao.js; o Guardiao audita os insumos (rateio) via ARCA-PIP-001, nao o acumulo final.

## NAO_APLICAVEL — regras estruturais (8)
...
- **Esperado:** Consolidacao em total de gramas conforme as medidas canonicas. Formulas da aba mensal: TOTAL DE MACONHA = MACONHA DOLAR*3 + MACONHA GRAMA; Total CRACK (gr) = CRACK GRAMA + CRACK PEDRA/4; TOTAL DE COCA
- **Diagnóstico(s):** —
- **Consumidor(es) real(is):** `Core/Constantes.js`
- **Motivo (auditabilidade_guardiao):** Conversao de drogas em gramas aplicada nas formulas da aba mensal; nao e auditoria do Guardiao.
...
- **Motivo (auditabilidade_guardiao):** Diagnostico deterministico do relatorio GXT (saida); nao e regra de auditoria de planilha.
...
- **Diagnóstico(s):** —
- **Consumidor(es) real(is):** `Entrada/EntradaManual.js`
- **Motivo (auditabilidade_guardiao):** Criterio discricionario de preenchimento (instrumento mais gravoso); sem fonte canonica no BO para auditar automaticamente.
...
- **Motivo (auditabilidade_guardiao):** Consolidacao analitica por orquestracao de plugins; o Guardiao audita insumos e rateios, nao o calculo final.
...
- **Diagnóstico(s):** —
- **Consumidor(es) real(is):** `Core/RegrasQualidade.js`, `Features/GuardiaoQualidade.js`, `Motor/PoliticaMeritoArmas.js`
- **Motivo (auditabilidade_guardiao):** Conceito estrutural de agregacao (chave DATA|MIKE|BOE); nao e regra de auditoria. Integridade coberta por ARCA-MIKE-004, ARCA-BOE-001 e ARCA-MIKE-002.
...
- **Motivo (auditabilidade_guardiao):** Validacao de construcao do agregado (fabrica); garantia de integridade interna do modelo, nao gera diagnostico de planilha.
...
- **Motivo (auditabilidade_guardiao):** Especificacao estrutural de layout fisico; a fragmentacao detectavel e coberta por ARCA-MIKE-004 (TUNEL_FRAGMENTADO). Consumida pelo corretor de tuneis.
...
- **Motivo (auditabilidade_guardiao):** Consumidor e o formulario (cliente, Entrada/Formulario.html), nao a planilha auditada.

## Lacunas apuradas

- Nenhuma lacuna apurada: todas as regras têm `status_cobertura` no JSON.
