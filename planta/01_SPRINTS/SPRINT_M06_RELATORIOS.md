...
- **COMPARATIVO_2026**
- **PIP (Compilador PIP | Ciclo 29–28)**
- **CPM (Compilador de Pontuação Mensal | Mês civil)**
- **RELATÓRIO DE ARMAS**
...
## 📋 Tabela de Inventário Real de Funções Executáveis no Código

| Relatório | Menu / Funções Executáveis Reais | Compilador Visual (Apresentação) | Motor / Métrica Interna | Aba(s) de Saída Real(is) | Fonte de Dados | Período | Métricas Obrigatórias | Status da Regra Visual |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
| **PIP** | `criarMenuPip_()`, `abrirMenuPipMensal()`, `abrirMenuPipLivre()`, `gerarPipAnual()` | `Compilador PIP.js` (`criarAbaResultado_()`) | Motor V2 (`SyntheonMetricas`, `SyntheonRanking`) | `PIP_<período>`, `PIP_ANUAL_2026`, `PIP_SELECAO_LIVRE` | Abas mensais (`JAN`..`DEZ`) + `EFETIVO` | Ciclo 29 do mês anterior a 28 do mês | Rank, Graduação, Matrícula, Nome Completo, Designação, Pontuação, Qtd. Oc. | **CONTRATO VISUAL A IMPLEMENTAR NO M06** (Possui cabeçalho verde `#D9EAD3` e formato `#,##0.00`) |
| **CPM** | `criarMenuCPM_()`, `abrirMenuCPMMensal()`, `abrirMenuCPMLivre()`, `gerarCPMAnual()` | `CPM – Compilador de Pontuação Mensal.js` (`criarAbaResultadoCPM_()`) | Motor V2 (`SyntheonRanking`, `pontosCPM`) | `CPM_<mes>`, `CPM_ANUAL_2026`, `CPM_SELECAO_LIVRE` | Abas mensais (`JAN`..`DEZ`) + `EFETIVO` | Mês Civil (dia 1º ao último dia) | Rank, Graduação, Matrícula, Nome Completo, Ocorrências, Pontuação | **CONTRATO VISUAL A IMPLEMENTAR NO M06** (Possui cabeçalho verde `#D9EAD3` e formato `#,##0.00`) |
| **ARMAS** | `criarMenuArmas_()`, `abrirMenuSelecaoLivre()` | `Compilador_Armas.js` (Inline no compilador) | Acumulador por Matrícula (conceitual `PluginArmas`) | `COMP_ARMAS_2026`, `COMP_ARMAS_<período>` | Abas mensais operacionais (`JAN`..`DEZ`) | Mensal / Seleção / Anual | Pelotão, Graduação, Matrícula, Policial, Score Acumulado (Armas) | **CONTRATO VISUAL A IMPLEMENTAR NO M06** (Tem cores básicas de Pelotão; falta escala de armas e GTAR) |
| **DROGAS** | `criarMenuDrogas_()`, `abrirMenuSelecaoLivreDrogas()` | `Compilador de Entorpecentes.js` (Inline no compilador) | Acumulador por Matrícula (conceitual `PluginEntorpecentes`) | `COMP_DROGAS_2026`, `COMP_DROGAS_<período>` | Abas mensais operacionais (`JAN`..`DEZ`) | Mensal / Seleção / Anual | Posição, Pelotão, Graduação, Matrícula, Policial, Maconha (g), Cocaína (g), Total (g), Ocorrências, BOEs | **CONTRATO VISUAL A IMPLEMENTAR NO M06** (Tem cores básicas de Pelotão; falta GTAR e formato `#,##0.00`g) |
...
- `TASK-M06.1-01C`: Inventário Fiel com Funções Executáveis Reais do Código (**CONCLUÍDA**).
