...
| :--- | :--- | :--- | :--- | :--- |
| **COMPARATIVO_2026** | `COMPARATIVO_2026` | `abrirMenuComparativo2026()`, `gerarComparativo2026Premium()` | Anual 2026 / Seleção Libre | **REFERÊNCIA VISUAL IMUTÁVEL** (Cores de Pelotão, GTAR, Oficiais, Escala de Armas, Carimbo e Formatos) |
| **PIP** | `PIP_<período>`, `PIP_ANUAL_2026`, `PIP_SELECAO_LIVRE` | `criarMenuPip_()`, `abrirMenuPipMensal()`, `abrirMenuPipLivre()`, `gerarPipAnual()` | Ciclo 29 do mês anterior a 28 do mês | Padronização visual com base no Comparativo, formato `#,##0.00`, mantendo a regra de ciclo 29–28 |
| **CPM** | `CPM_<mes>`, `CPM_ANUAL_2026`, `CPM_SELECAO_LIVRE` | `criarMenuCPM_()`, `abrirMenuCPMMensal()`, `abrirMenuCPMLivre()`, `gerarCPMAnual()` | Mês Civil (dia 1º ao último dia) | Padronização visual com base no Comparativo, formato `#,##0.00`, mantendo a regra de mês civil |
| **ARMAS** | `COMP_ARMAS_2026`, `COMP_ARMAS_<período>` | `criarMenuArmas_()`, `abrirMenuSelecaoLivre()` | Mensal / Seleção / Anual | Pelotões, GTAR (`#660000`), Oficiais (`#000066`), Escala Oficial de Armas e valor zero em vermelho (`#FF0000`) |
| **DROGAS** | `COMP_DROGAS_2026`, `COMP_DROGAS_<período>` | `criarMenuDrogas_()`, `abrirMenuSelecaoLivreDrogas()` | Mensal / Seleção / Anual | Pelotões, GTAR (`#660000`), Oficiais (`#000066`), alinhamentos à direita e gramaturas formatadas (`#,##0.00`g) |
...
2. **Preservação do `COMPARATIVO_2026`:** O relatório `COMPARATIVO_2026` é a referência estática e soberana de design. Nenhuma alteração estética será feita sobre ele; apenas testes de regressão serão adicionados.
...
- **`TASK-M06.2-04`**: Padronizar ARMAS (pelotões, GTAR, escala oficial e zero em vermelho).
- **`TASK-M06.2-05`**: Padronizar DROGAS (pelotões, GTAR, alinhamentos e gramaturas `#,##0.00`g).
